...
import { BellIcon, SearchIcon, Pencil, Trash2 } from "lucide-react";
            ...
            <div className="p-0 overflow-x-auto">
              <table className="w-full table-fixed border-collapse">
                <colgroup>
                  ...
                  <col className="w-[5%]" />  {/* Gender */}
                  <col className="w-[11%]" /> {/* Email */}
                  <col className="w-[7%]" />  {/* Contact */}
                  <col className="w-[8%]" />  {/* Last Visit */}
                  <col className="w-[5%]" />  {/* Edit/Delete */}
                </colgroup>

                <thead>
                  <tr className="border-b">
                    {["Patient Name", "Age", "Gender", "Email", "Contact No.", "Last Visit", ""].map(
                      (head) => (
                        <th
                          key={head}
                ...
                <tbody>
                  {patientsData.map((p, i) => (
                    <tr key={i} className="border-b border-gray-200 hover:bg-gray-50">
                      <td className="py-3 pl-8 font-medium text-gray-900 text-sm truncate">{p.name}</td>
                      <td className="py-3 px-4 text-gray-700 text-sm truncate">{p.age}</td>
                      <td className="py-3 px-4 text-gray-700 text-sm truncate">{p.gender}</td>
                      <td className="py-3 px-4 text-gray-700 text-sm truncate">{p.email}</td>
                      <td className="py-3 px-4 text-gray-700 text-sm truncate">{p.contact}</td>
                      <td className="py-3 px-4 text-gray-700 text-sm truncate">{p.lastVisit}</td>
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-3">
                          <button
                            type="button"
                            className="p-1 rounded hover:bg-gray-100 text-gray-600 hover:text-[#30B8DE]"
